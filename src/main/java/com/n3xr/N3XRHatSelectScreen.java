...
/**
 * Screen list pilihan hat lokal, plus panel preview 3D player doll
 * di sebelah kanan. Struktur & gaya identik dengan N3XRCapeSelectScreen
 * (lihat komentar di file itu soal alasan pakai overload bawaan
 * InventoryScreen.drawEntity, bukan Quaternion manual).
 */
public class N3XRHatSelectScreen extends Screen {
        ...
        private List<N3XRHatManager.HatEntry> hats;
        ...
        public N3XRHatSelectScreen(Screen parent) {
                super(Text.literal("Select Hat"));
                this.parent = parent;
        }

        @Override
        protected void init() {
                hats = N3XRHatManager.getAvailableHats();
        ...
        public boolean mouseClicked(double mouseX, double mouseY, int button) {
                ...
                for (int i = 0; i < rowRects.size(); i++) {
                        ...
                        if (mouseX >= r[0] && mouseX <= r[0] + r[2] && mouseY >= r[1] && mouseY <= r[1] + r[3]) {
                                if (i == 0) {
                                        N3XRConfig.hatSelectedKey = null;
                                } else {
                                        N3XRConfig.hatSelectedKey = hats.get(i - 1).key();
                                }
                                return true;
                        }
                }
        ...
        public void render(DrawContext context, int mouseX, int mouseY, float delta) {
                ...
                Text title = Text.literal("Select Hat").styled(s -> s.withBold(true));
                ...
                for (int i = 0; i < rowRects.size(); i++) {
                        ...
                        String key = (i == 0) ? null : hats.get(i - 1).key();
                        String label = (i == 0) ? "None (disable hat)" : hats.get(i - 1).displayName();

                        boolean isSelected = java.util.Objects.equals(N3XRConfig.hatSelectedKey, key);
